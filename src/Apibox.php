...
class Apibox {
	...
	public function create_folder($name, $parent_id, $access_token) {
		...
		if(isset($result['status']) && intval($result['status']) == 409){
			return array('status' => 'ok', 'id' => $result['context_info']['conflicts'][0]['id']);
		}elseif(isset($result['id'])){
			return array('status' => 'ok', 'id' => $result['id']);
		}else{
			return array('status' => 'error', 'msg' => 'Unknown error occured');
		}
	}
